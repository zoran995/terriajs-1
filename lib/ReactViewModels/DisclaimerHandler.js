...
export default class DisclaimerHandler {
  ...
  _handleInitialMessage(catalogItem, successCallback) {
    ...
    if (
      keySpecified &&
      this.terria.localPropertyService.get(catalogItem.initialMessage.key)
    ) {
      successCallback();
      return;
    }
    ...
    if (catalogItem.initialMessage.confirmation) {
      ...
      if (keySpecified) {
        this.terria.localPropertyService.set(
          catalogItem.initialMessage.key,
          true
        );
      }
  ...
  _executeCallbacksForKey(key) {
    ...
    this.terria.localPropertyService.set(key, true);
  }
