...
import prettifyProjectionGK from "../Map/prettifyProjectionGK";
...
export default class MouseCoords {
  constructor() {
    ...
    this.proj4ProjectionUTM = "+proj=utm +ellps=GRS80 +units=m +no_defs";
    this.proj4ProjectionGK =
  ...
  toggleUseProjection() {
    if (this.projection === projection.WGS84) {
      this.projection = projection.UTM;
    } else if (this.projection === projection.UTM) {
      this.projection = projection.GK;
    } else {
  ...
  cartographicToFields(coordinates, errorBar) {
    ...
    var prettyProjection;
    if (this.projection === projection.UTM) {
      prettyProjection = prettifyProjection(
        longitude,
        latitude,
        this.proj4ProjectionUTM,
        this.proj4longlat,
        this.projectionUnits
      );
      this.zone = prettyProjection.utmZone;
      this.north = prettyProjection.north;
      this.east = prettyProjection.east;
    } else if (this.projection === projection.GK) {
      prettyProjection = prettifyProjectionGK(
        14.3534,
        45.2344,
        this.proj4ProjectionGK,
        this.proj4longlat,
      ...
      this.zone = prettyProjection.zone;
      this.north = prettyProjection.north;
      this.east = prettyProjection.east;
    }
