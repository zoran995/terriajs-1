...
var Terria = function(options) {
  ...
  /**
   * Gets or sets whether to check data for feature info. Default true. This property is observable.
   * Used by measure and drawing tool.
   * @type {Boolean}
   */
  this.shouldCheckFeatureInfo = true;
  /**
  ...
  /**
   * Array of pointEntities
   */
  this.drawnEntities = new CustomDataSource("drawnEntities");

  this.dataSources.add(this.drawnEntities);

  /**
   * Array of lineEntities
   */
  this.drawnLineEntities = new CustomDataSource("drawnLines");

  /**
   * Array of polygonEntities
   */
  this.drawnPolygonEntities = new CustomDataSource("drawnPolygons");

  /**
   * Should drawn features indicate pointer cursor, like other features.
   * Currently it is not supported to dynamically change interactive property, see https://github.com/Leaflet/Leaflet/issues/5442.
   * This is not important for cesium, since interactive option is not used in cesium
   * @type {Boolean}
   */
  this.drawnFeaturesInteractive = false;

  /**
   * Store current drawing mode. Default notDefined.
   * @type {DrawingMode}
   */
  this.currentDrawingMode = DrawingMode.notDefined;
  /**
