...
var WebFeatureServiceCatalogItem = function(terria) {
   ...
   *
   */
  this._attributeTable = undefined;
...
defineProperties(WebFeatureServiceCatalogItem.prototype, {
  ...
  /**
   * Gets the data attribute table associated with this catalog item.
...
WebFeatureServiceCatalogItem.prototype._load = function() {
  ...
  var that = this;
  return that._geoJsonItem
    .load()
    .then(function() {
      that.rectangle = that._geoJsonItem.rectangle;
    })
    .then(function() {
      that._attributeTable = that._geoJsonItem.attributeTable;
    });
};
