    ...
    return when(that.data, function(data) {
      if (typeof Blob !== "undefined" && data instanceof Blob) {
        return readText(data).then(function(text) {
          return loadTableFromCsv(that, text).then(function() {
            that.initializeAttributeTable();
          });
        });
      } else if (typeof data === "string") {
        return loadTableFromCsv(that, data).then(function() {
          that.initializeAttributeTable();
        });
      } else if (data instanceof TableStructure) {
        that.applyTableStyleColumnsToStructure(that._tableStyle, data);
        return that.initializeFromTableStructure(data).then(function() {
          that.initializeAttributeTable();
        });
      } else {
