...
TableCatalogItem.prototype.initializeAttributeTable = function() {
  var item = this;
  if (defined(item.dataSource)) {
    var guid = item.dataSource.guid;
    const columns = item.tableStructure.getColumnNamesAndTypes();
    var attributeTable = new AttributeTable(item, columns, guid, item.terria);
