...
var UserDrawing = function(options) {
  ...
  this.shouldRender = defaultValue(options.shouldRender, true);
  /**
   * Instance of Terria
   * @type {Terria}
   * @default undefined
...
/**
 * This is used for MeasureTool for retrieving point coordinate.
 * Doesn't use _prepareToAddNewPoint since it only allow drawing of one point
 */
UserDrawing.prototype.drawPoint = function() {
  this.dragHelper.setUp();
  // If we have finished a polygon, don't allow more points to be drawn. In future, perhaps support multiple polygons.
  if (this.inDrawMode || this.closeLoop) {
    // Do nothing
    ...
    .subscribe(function(pickedFeatures) {
      when(pickedFeatures.allFeaturesAvailablePromise, function() {
        if (defined(pickedFeatures.pickPosition)) {
          ...
          if (that.pointEntities.entities.values.length >= 1) {
            that.pointEntities.entities.removeById(
              that.pointEntities.entities.values[0].id
            );
          }
          // add new point
          that._addPointToPointEntities(
