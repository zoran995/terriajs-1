...
var AttributeTable = require("../Map/AttributeTable");
        ...
        if (defined(that.data)) {
          return when(that.data, function(data) {
            ...
            return when(promise, function(json) {
              that.data = json;
              return updateModelFromData(that, json).then(function(a) {
                that.initializeAttributeTable();
              });
            }).otherwise(function() {
              throw new TerriaError({
                sender: that,
...
GeoJsonCatalogItem.prototype.initializeAttributeTable = function() {
  var item = this;
  if (defined(item.dataSource)) {
    var guid = item.dataSource.guid;
    if (item.dataSource.entities.values.length > 0) {
      const cn = [];
      const columns =
        item.dataSource.entities.values[0].properties.propertyNames;
      for (var i = 0; i < columns.length; i++) {
        cn.push({ name: columns[i], type: undefined });
      }
      var attributeTable = new AttributeTable(item, cn, guid, item.terria);
      item._attributeTable = attributeTable;
    }
  }
};
