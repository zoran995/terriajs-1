...
DrawingPoint.prototype._changeInteractivity = function(drawing) {
  console.log(drawing.drawnEntities.entities.values[0].point);
  drawing.drawnEntities.entities.values[0].point.interactive = false;
  console.log(drawing.drawnEntities.entities.values[0].point.interactive);
...
DrawingPoint.prototype.updateTitle = function(feature, title) {
  const entity = this.drawnEntities.entities.getById(feature.id);
  const entityTitle = entity.point || entity.line || entity.polygon;
  if (defined(entityTitle)) {
    entityTitle.title = title || entityTitle.text;
  }
};
