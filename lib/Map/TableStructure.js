...
/**
 * Get the column names with data types.
 *
 * @returns {String[Object]} Array of column names with data types.
 */
TableStructure.prototype.getColumnNamesAndTypes = function() {
  var result = [];
  for (var i = 0; i < this.columns.length; i++) {
    result.push({ name: this.columns[i].name, type: this.columns[i].type });
  }
  return result;
};
